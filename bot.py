...
1. Данные медиаплана переданы в формате "Заголовок: Значение". Читай каждую строку внимательно.
2. НИКОГДА не указывай на отсутствие данных если они присутствуют в файле — даже если формат неудобный.
3. Если данные есть, но их сложно интерпретировать — пиши ВНИМАНИЕ, не КРИТИЧНО и не "отсутствует".
4. Если поле присутствует хотя бы в части строк — не считай его отсутствующим глобально.
5. Перед тем как написать "поле отсутствует" — перечитай данные ещё раз. Ошибочное КРИТИЧНО хуже пропущенного замечания.
6. Скрытые строки и столбцы не проверяются — только то что передано в тексте.
7. Если данных недостаточно для уверенного вывода — пиши "недостаточно данных для оценки", не придумывай проблем.

Проверь медиаплан по 5 блокам:

БЛОК 1 — СТРУКТУРА И ФОРМАТ
- КРИТИЧНО: Полнота обязательных полей (площадка, формат, период, ЦА, ГЕО, показы/клики/охват, CPM/CPC/CPV, бюджет по строке, итоговый бюджет). Фиксируй только поля которых нет ни в одной строке.
- ВНИМАНИЕ: Разные написания одной и той же площадки или формата в разных строках.
- ЗАМЕЧАНИЕ: Единый формат дат и чисел.

БЛОК 2 — БЮДЖЕТ И ФИНАНСЫ
- КРИТИЧНО: Видимые ошибки в расчётах — итог не совпадает с суммой строк если оба значения присутствуют.
- КРИТИЧНО: НДС — если ставка указана, проверь единообразие. Для Казахстана НДС = 16%.
- ВНИМАНИЕ: Несколько валют без указания курса конвертации.
- ВНИМАНИЕ: Модели закупки (CPM/CPC/CPL) — если модель указана, проверь соответствие задаче.

БЛОК 3 — МЕДИАПОКАЗАТЕЛИ
- КРИТИЧНО: Явные математические несоответствия — например показы × CTR ≠ клики если оба значения есть.
- ВНИМАНИЕ: CTR, CPM, CPC которые явно выбиваются из рыночных норм (CTR баннера > 3% — подозрительно).
- ВНИМАНИЕ: Охват превышает реалистичный размер ЦА.

БЛОК 4 — ТАРГЕТИНГИ И АУДИТОРИЯ
- КРИТИЧНО: ГЕО отсутствует полностью ни в одной строке.
- ВНИМАНИЕ: Разные ГЕО в строках без явного обоснования.
- ВНИМАНИЕ: Возраст или пол ЦА не указан ни в одной строке.

БЛОК 5 — СРОКИ И ПЕРИОДЫ
- КРИТИЧНО: Даты окончания раньше дат начала.
- ВНИМАНИЕ: Видимые пересечения или разрывы между этапами кампании.
- ВНИМАНИЕ: Даты выходят за рамки разумного планового периода.

КЛАССИФИКАЦИЯ:
- КРИТИЧНО — явная ошибка с доказательством из данных. Исправить до утверждения.
- ВНИМАНИЕ — существенное замечание, влияет на качество. Рекомендуется исправить.
...
[если есть:]
🔴 *КРИТИЧНО — [заголовок]*
Описание: [что именно не так — со ссылкой на конкретные данные из файла]
...
def extract_excel_content(file_bytes: bytes, filename: str) -> str:
    """Извлекает содержимое Excel с сохранением структуры заголовков."""
    name = filename.lower()
    result = []
    try:
        if name.endswith('.xls'):
            ...
            for sheet in wb.sheets():
                ...
                headers = rows_raw[0]
                result.append("Заголовки столбцов: " + " | ".join(h for h in headers if h))
                result.append("")
