...
ОБЩИЕ ПАРАМЕТРЫ — проверяй по всему документу:
- ВНИМАНИЕ: Модели закупки (CPM/CPC/CPL/CPI) соответствуют целям кампании и KPI клиента. Сверить с брифом.
- ВНИМАНИЕ: Если МП содержит несколько вкладок — сводная вкладка корректно агрегирует данные из всех листов.
- ВНИМАНИЕ: Валюта МП соответствует бюджету клиента. При нескольких валютах — курс конвертации зафиксирован с датой.
...
- ВНИМАНИЕ: Модель закупки не соответствует задаче клиента.

БЛОК 3 — МЕДИАПОКАЗАТЕЛИ
- КРИТИЧНО: Логическая связность — показы × CTR ≠ клики, бюджет / показы × 1000 ≠ CPM (если оба значения присутствуют).
- ВНИМАНИЕ: Реалистичность прогнозных показателей — CTR баннера > 3% подозрительно, CPM/CPC вне рыночных норм.
- ВНИМАНИЕ: Ставки — соответствие рекомендациям РРС, сравнение с предыдущим флайтом.
- ВНИМАНИЕ: Охват превышает реалистичный размер ЦА. Частота выше 10 — требует обоснования.
...
- ВНИМАНИЕ: Дедлайны площадок — материалы за 2-5 рабочих дней до старта.
...
☐ Версия файла актуальна (не черновик и не устаревшая итерация)?
☐ Файл назван по стандарту агентства?
☐ Лишние черновые вкладки удалены из финального файла?
